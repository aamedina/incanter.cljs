(ns incanter.impl.pprint
  (:require [clojure.string :refer [join]]
            [goog.string :as gstr]
            [goog.string.format]
            [incanter.impl.width :refer [get-widths]])
...
(defn- generate-rows-and-fields
  ...
  (let [
        ...
        rows (cond
               ...
               (map? (first table))
               (map #(map (fn [k] (get % k)) fields) table)
               (map? table) table
               :else (rest table))
  ...
  (let [headers (map #(if (keyword? %) (name %) (str %)) fields)
        ...
        fmt-row (fn [row]
                  (map-indexed
                   ...
                   row))
        wrap-row (fn [row strings] (let [[beg mid end] strings]
...
(defn- escape-newline [newline] (goog.string/escapeString newline))
